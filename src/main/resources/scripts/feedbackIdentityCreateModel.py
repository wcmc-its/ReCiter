    ...
    try:
        ...
        with engine.connect() as connection:
            query = '''
                SELECT 
                o.id AS articleId, 
                scoreCites AS feedbackScoreCites, 
                scoreCoauthorName AS feedbackScoreCoAuthorName,
                scoreEmail AS feedbackScoreEmail, 
                scoreInstitution AS feedbackScoreInstitution,
                scoreJournal AS feedbackScoreJournal, 
                scoreJournalSubfield AS feedbackScoreJournalSubField,
                scoreKeyword AS feedbackScoreKeyword, 
                scoreOrcid AS feedbackScoreOrcid, 
                scoreOrcidCoauthor AS feedbackScoreOrcidCoAuthor,
                scoreOrganization AS feedbackScoreOrganization, 
                scoreTargetAuthorName AS feedbackScoreTargetAuthorName,
                scoreYear AS feedbackScoreYear, 
                s.articleCountScore,
                ...
                case 
                  when o.userAssertion = 'ACCEPTED' then countAccepted - 1
                  else countAccepted
                 end as countAccepted,
                case 
                  when o.userAssertion = 'REJECTED' then countRejected - 1
                  else countRejected
                 end as countRejected,                 
                o.userAssertion
            FROM 
        ...
        # Save data to 'feedbackIdentityScoringInput.json' (optional, can be removed if not needed)
        json_data = data.to_dict(orient='records')
        with open('feedbackIdentityScoringInput.json', 'w') as f:
            json.dump(json_data, f, indent=4)
        print("Data saved to 'feedbackIdentityScoringInput.json '")
...
def preprocess_data(data):
    ...
    joblib.dump(scaler, 'feedbackIdentityScaler.save')
    print("Scaler saved as 'feedbackIdentityScaler.save'")
...
def build_and_train_model(X_scaled, y_encoded):
    ...
    # Save the model
    model.save('feedbackIdentityModel.keras')
    print('Model saved as "feedbackIdentityModel.keras"')
