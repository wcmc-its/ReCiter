        ...
        protected String getStrategySpecificKeyword(Identity identity) {
            if (identity.getPrimaryName() != null || (identity.getAlternateNames() != null && !identity.getAlternateNames().isEmpty())) {
                ...
                if(capitalizeFirstMiddle.length() >= 2) {
                    first = identity.getPrimaryName().getLastName() + " " + capitalizeFirstMiddle.substring(0, 2) + "[au] OR "
                         + identity.getPrimaryName().getLastName() + " " + capitalizeFirstMiddle.substring(0, 2) + "[ir]";
                }
                ...
                while(iter.hasNext()) {
                    ...
                    capitalizeFirstMiddle = ReCiterStringUtil.deAccent(combinedFirstMiddleName.chars().filter(Character::isUpperCase)
                                ...
                                .collect(Collectors.joining()));
                    if(capitalizeFirstMiddle.length() >= 2) {
                        alternateNameWithInitial = alternateName.getLastName() + " " + capitalizeFirstMiddle.substring(0, 2) + "[au] OR "
                                                 + alternateName.getLastName() + " " + capitalizeFirstMiddle.substring(0, 2) + "[ir]";
                    }
