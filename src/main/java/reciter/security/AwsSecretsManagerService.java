 ...
 * 
 *//**
	 * @author mjangari Helps in retrieving the Keys and values from the AWS Secrets
	 *         Manager.
	 *//*
		 * @Service public class AwsSecretsManagerService {
		 * 
		 * private static final Logger log =
		 * LoggerFactory.getLogger(AwsSecretsManagerService.class);
		 * 
		 * private static final ObjectMapper objectMapper = new ObjectMapper();
		 * 
		 * 
		 * @Value("${aws.secretsmanager.region}") private String secretManagerRegion;
		 * 
		 * public Optional<SecretsManagerClient> createSecretsManagerClient() { return
		 * Optional.of( SecretsManagerClient.builder() .region(secretManagerRegion!=null
		 * ?Region.of(secretManagerRegion):Region.of("us-east-1")) // in any case if
		 * SecretsManager is null then read from the us-east-1 region
		 ...
		 * .orElseThrow(() -> new
		 * IllegalStateException("SecretsManagerClient could not be created: region is missing"
		 * ));
		 * 
		 * log.info("secretName pulled from application.properties" + secretName); //
		 * Fetch the secret from AWS Secrets Manager GetSecretValueRequest valueRequest
		 ...
		 * 
		 * try { if(secretJson!=null && !secretJson.equalsIgnoreCase("")) { // Parse the
		 * secret string into a JsonNode (Jackson's tree model) JsonNode jsonNode =
		 * objectMapper.readTree(secretJson); return jsonNode; } else { throw new
		 * Exception("Secret not found or is empty"); }
		 * 
		 ...
		 * return clientSecrets; // Return the client secret based on the clientId }
		 * 
		 * public JsonNode getSecretValueFromSecretsManager(String secretName, String
		 * secretKey) { JsonNode secretValueJson = getSecrets(secretName); if
		 * (secretValueJson != null && secretKey!=null &&
		 * !secretKey.equalsIgnoreCase("")) { return secretValueJson.get(secretKey); }
		 * return null; } }
		 * 
		 */